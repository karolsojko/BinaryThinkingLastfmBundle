...
class LastfmAPIClientFactoryTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testGetGeoClient()
    {
        $geoClient = LastfmAPIClientFactory::getGeoClient($this->apiKey, $this->apiSecret);
        $this->assertInstanceOf('BinaryThinking\LastfmBundle\Lastfm\Client\Method\GeoMethodsClient',
            $geoClient, 'wrong client delivered');
    }

    public function testGetClientForGeo()
    {
        $geoClient = LastfmAPIClientFactory::getClient('geo', $this->apiKey, $this->apiSecret);
        $this->assertInstanceOf('BinaryThinking\LastfmBundle\Lastfm\Client\Method\GeoMethodsClient',
            $geoClient, 'wrong client delivered');
    }
